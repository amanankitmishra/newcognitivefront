...
import { createSalesOrder, fetchSalesOrder, editSalesOrder, deleteSalesOrder } from 'src/utility/api';
...
const SalesOrder = () => {
  ...
  const handleViewSalesOrder = (clientId) => {
    Router.push(`/sales-order/view?id=${clientId}`);
  };
  ...
  const handleEditSalesOrder = async (id, editedData) => {
    try {
      const response = await editSalesOrder(id, editedData)
  ...
  const handleConfirmationDialogConfirm = async () => {
    try {
      ...
      await deleteSalesOrder(deleteProposalId);
