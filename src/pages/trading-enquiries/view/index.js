...
import { fetchTradingEnquiryById } from 'src/utility/api'
...
const ViewEnquiry = () => {
  ...
  const getEnquiry = async () => {
    const response = await fetchTradingEnquiryById(id)
  ...
  return (
    <div>
      <Grid container>
        <Grid item xs={12}>
          <Card>
            <CardHeader title={enquiry.quotationNumber}></CardHeader>
            <CardContent>
              <Grid container>
                <Grid item xs={6}>
                  <Table>
              ...
              <Button sx={{ margin: '20px' }} onClick={() => handleViewClient(enquiry.clientId)}>
                View Client
              </Button>
            </CardContent>
          </Card>
