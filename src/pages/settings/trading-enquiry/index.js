...
import {
  fetchTradingEnquiryNumbers,
  addTradingEnquiryNumber,
  markActiveTradingEnquiryNumber,
  deleteTradingEnquiryNumber
} from 'src/utility/api'
import Tooltip from '@mui/material/Tooltip'

const TradingEnquirySettings = () => {
  ...
  const handleMarkActive = async id => {
    try {
      const response = await markActiveTradingEnquiryNumber(id)
  ...
  const handleDeleteTEN = async id => {
    try {
      const response = await deleteTradingEnquiryNumber(id)
  ...
  const handleATENSave = async () => {
    try {
      const response = await addTradingEnquiryNumber(formData)
...
TradingEnquirySettings.acl = {
  action: 'manage',
  subject: 'settings'
}

export default TradingEnquirySettings
