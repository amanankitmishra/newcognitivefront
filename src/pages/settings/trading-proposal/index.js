...
import {
  fetchTradingProposalNumbers,
  addTradingProposalNumber,
  markActiveTradingProposalNumber,
  deleteTradingProposalNumber
} from 'src/utility/api'
import Tooltip from '@mui/material/Tooltip'

const TradingProposalSettings = () => {
  ...
  const getTradingProposalNumbers = async () => {
    const response = await fetchTradingProposalNumbers()
  ...
  const handleMarkActive = async id => {
    try {
      const response = await markActiveTradingProposalNumber(id)
  ...
  const handleDeleteTPN = async id => {
    try {
      const response = await deleteTradingProposalNumber(id)
  ...
  const handleATPNSave = async () => {
    try {
      const response = await addTradingProposalNumber(formData)
...
TradingProposalSettings.acl = {
  action: 'manage',
  subject: 'settings'
}

export default TradingProposalSettings
